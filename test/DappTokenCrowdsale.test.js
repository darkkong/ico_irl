...
const TokenTimelock = artifacts.require("openzeppelin-solidity/TokenTimelock");
...
contract(
  ...
  ([
    ...
    let token, crowdsale, openingTime, closingTime, releaseTime;
    ...
    describe("accepting payments", () => {
      it("should accept payments", async () => {
        ...
        await crowdsale.sendTransaction({ value: value, from: investor1 })
          .should.be.fulfilled;
      ...
      describe("finalizing the crowdsale", () => {
        describe("when the goal is not reached", () => {
          ...
          it("allows the investor to claim refund", async () => {
            await crowdsale.claimRefund(investor2, { from: investor2 }).should
              .be.fulfilled;
        ...
        describe("when the goal is reached", () => {
          ...
          it("handles goal reached", async () => {
            ...
            await token.transfer(investor2, 1, { from: investor1 }).should.be
              .fulfilled;
            ...
            let foundersTimelockBalance = await token.balanceOf(
            ...
            assert.equal(
              foundersTimelockBalance.toString(),
              foundersAmount.toString()
            );

            // Foundation
            const foundationTimelockAddress =
              await crowdsale.foundationTimelock();
            let foundationTimelockBalance = await token.balanceOf(
              foundationTimelockAddress
            );
            foundationTimelockBalance = foundationTimelockBalance.toString();
            foundationTimelockBalance =
              foundationTimelockBalance / 10 ** decimals;
            ...
            let partnersTimelockBalance = await token.balanceOf(
            ...
            assert.equal(
              partnersTimelockBalance.toString(),
              partnersAmount.toString()
            );

            // Can't withdraw from timelocks
            const foundersTimelock = await TokenTimelock.at(
              foundersTimelockAddress
            );
            ...
            await foundationTimelock
              .release()
              .should.be.rejectedWith(EVMRevert);
            const partnersTimelock = await TokenTimelock.at(
              partnersTimelockAddress
            );
      ...
      describe("token distribution", () => {
        ...
        it("tracks token distribution correctly", async () => {
          tokenSalePercentageResult
            ...
            .should.equal(
              tokenSalePercentage,
              "has correct tokenSalePercentage"
            );
          foundersPercentageResult
            .toNumber()
